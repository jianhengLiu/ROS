        ...
        cal_count(0)
{
  ...
  //********stanley********//
  private_nh_.param("/move_base/useMagnetometer",useMagnetometer_,false);
  private_nh_.param("/move_base/auto_update_",auto_update_,false);
  private_nh_.param("/move_base/particle_range",particle_range,1.0);
  private_nh_.param("/move_base/angle_tolerance",angle_tolerance_,1.0);
  private_nh_.param("/map/angle_init", angle_init, float(0.0));
  //********stanley********//
  ...
  //********stanley********//
  magnetometer_ = nh_.subscribe("magnetometer",1,&AmclNode::magnetometerCallback,this);
  //********stanley********//
...
void AmclNode::magnetometerCallback(const std_msgs::Float32MultiArray& msg)
{
  angle_now.data = msg.data[1];
}
...
/*
  ...
  p.v[1] = MAP_WYGY(map, free_point.second);
  if(useMagnetometer_){
    p.v[2] = angle_now.data - angle_init;
...
AmclNode::laserReceived(const sensor_msgs::LaserScanConstPtr& laser_scan)
{
  ...
  if(pf_init_)
  {
    ...
    auto_update_ = auto_update_ || m_force_update;
    */
   bool update;
    if(auto_update_){
...
AmclNode::handleInitialPoseMessage(const geometry_msgs::PoseWithCovarianceStamped& msg)
{
  ...
  //********stanley********//
  std_msgs::Float32 angle_flag;
  pf_matrix_t pf_init_pose_cov = pf_matrix_zero();
  if (useMagnetometer_){
    pf_init_pose_mean.v[2] = angle_now.data - angle_init;
    if (pf_init_pose_mean.v[2] > M_PI)
      pf_init_pose_mean.v[2] = 2* M_PI - pf_init_pose_mean.v[2];
    else if (pf_init_pose_mean.v[2] < -M_PI)
      pf_init_pose_mean.v[2] = 2* M_PI + pf_init_pose_mean.v[2];

    ROS_WARN("*****pf_init_pose_mean.v[2]=%f\n",pf_init_pose_mean.v[2]);
    // Copy in the covariance, converting from 6-D to 3-D
    for(int i=0; i<2; i++){
      for(int j=0; j<2; j++){
        if (i==j){
            pf_init_pose_cov.m[i][j] = particle_range*particle_range;
        }
        else
          pf_init_pose_cov.m[i][j] = 0;
      }
    }
    pf_init_pose_cov.m[2][2] = angle_tolerance_;
  }
  else{
